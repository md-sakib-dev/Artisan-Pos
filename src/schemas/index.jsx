...
export const  cardPay=Yup.object({
    group: Yup.string().required('Please select a type'), 
    newgroup: Yup.string().required('Please select a type'),
    price: Yup.string()
    .matches(/^\d+(\.\d{1,2})?$/, 'Price must be a valid number with up to two decimal places')
    .matches(/^[^,]*$/, 'Price cannot contain commas')
    .required('Price is required'),
...
export const  mobilePaySchema=Yup.object({
    group: Yup.string().required('Please select a type'), 
    newgroup: Yup.string().required('Please select a type'),
    price: Yup.string()
    .matches(/^\d+(\.\d{1,2})?$/, 'Price must be a valid number with up to two decimal places')
...
export const giftvoucherSchema= Yup.object({
    chalan: Yup.string().matches(/^[0-9]+$/, 'Quantity must contain only numbers').required('chalan no required'),
    recieve: Yup.string().required('Please select  a type'), 
})
export const chalanSchema= Yup.object({
    chalan: Yup.string().matches(/^[0-9]+$/, 'Quantity must contain only numbers').required('chalan no required'),
    recieve: Yup.string().required('Please select  a type'), 
})
